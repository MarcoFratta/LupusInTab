...
function addPlayer() { localPlayers.value.push({ name: t('forms.playerPlaceholder', { number: localPlayers.value.length + 1 }) }); localNum.value = localPlayers.value.length; }
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      <div class="max-h-80 overflow-auto space-y-2 pr-1.5">
        <div v-for="(p, idx) in localPlayers" :key="idx" class="flex items-center gap-2.5 p-2.5 bg-neutral-900/60 border border-neutral-800/40 rounded-lg">
          ...
          <input type="text" v-model="p.name" class="flex-1 px-2 py-1 bg-transparent border border-neutral-800/40 rounded text-neutral-100 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/40 focus:border-indigo-500/40" :placeholder="t('forms.playerPlaceholder', { number: idx + 1 })" />
