...
<template>
  <div class="space-y-6" v-if="roleDef">
    <div class="bg-gradient-to-br from-neutral-900/80 to-neutral-800/60 border border-neutral-700/40 rounded-xl p-6 md:p-8">
      <div class="text-center space-y-6">
        <div>
          ...
          <div class="text-3xl md:text-4xl font-extrabold tracking-tight mb-3" 
               :style="{ color: getFactionConfig(roleDef.team)?.color || '#e5e7eb' }">
            {{ roleDef.name }}
          </div>
          <div v-if="roleDef.description" class="text-neutral-300 text-sm md:text-base leading-relaxed max-w-md mx-auto mb-6">
            {{ roleDef.description }}
          </div>
          
          <div class="inline-flex items-center gap-3 px-4 py-2 rounded-xl bg-neutral-800/40 border border-neutral-700/40">
            <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: getFactionConfig(roleDef.team)?.color || '#e5e7eb' }"></div>
            <span class="text-neutral-200 font-medium">La tua fazione</span>
            <FactionLabel :team="roleDef.team" :labelText="getFactionConfig(roleDef.team)?.displayName || roleDef.team" />
          </div>
        </div>

        <div v-if="(props.knownRoleAllies && props.knownRoleAllies.length) || (props.knownTeamAllies && props.knownTeamAllies.length)" class="pt-6 border-t border-neutral-700/40">
          <div class="text-center mb-6">
            <h4 class="text-lg md:text-xl font-semibold text-neutral-200 mb-2">I tuoi alleati</h4>
            <div class="w-12 h-0.5 bg-gradient-to-r from-violet-500/50 to-fuchsia-500/50 mx-auto rounded-full"></div>
          </div>
          
          <div v-if="props.knownRoleAllies && props.knownRoleAllies.length" class="mb-6 last:mb-0">
            <div class="text-neutral-300 text-sm font-medium mb-3 flex items-center gap-2">
    ...
    <div class="pt-2">
      <button 
        class="btn btn-accent w-full px-6 py-3 text-base font-semibold rounded-xl transform hover:scale-105 active:scale-95 transition-all duration-200 shadow-lg shadow-violet-500/25"
        @click="handleNext"
      >
  ...
  <div v-else class="space-y-6">
    <div class="bg-neutral-900/60 border border-neutral-800/40 rounded-xl p-6 md:p-8 text-center">
      <div class="space-y-4">
        <div class="w-16 h-16 mx-auto bg-gradient-to-br from-red-500/20 to-orange-500/20 border border-red-500/30 rounded-full flex items-center justify-center">
          <svg class="w-8 h-8 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
          </svg>
        </div>
        <div>
          <div class="text-2xl md:text-3xl font-extrabold tracking-tight text-neutral-200 mb-2">Ruolo sconosciuto</div>
          <div class="text-neutral-400 text-sm md:text-base">{{ player?.roleId }}</div>
        </div>
      </div>
    </div>
    <div class="pt-2">
