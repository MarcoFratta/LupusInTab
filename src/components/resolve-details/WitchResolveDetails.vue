...
const investigationEvents = computed(() => {
    ...
    return playerAction && playerAction.type === 'witch_check' ? [playerAction] : [];
});

const witchPlayer = computed(() => {
    const playerId = props.entry?.playerId;
    return playerId ? props.gameState.players.find(p => p.id === playerId) : null;
});
