...
<template>
  <div class="text-center p-2 space-y-1">
    ...
    <div>
      <div class="text-neutral-100 font-medium text-xs truncate max-w-full" :title="props.player?.name || t('roleStates.player')">{{ props.player?.name || t('roleStates.player') }} {{ reason === 'alive' ? t('roleStates.alive') : t('roleStates.blocked') }}</div>
