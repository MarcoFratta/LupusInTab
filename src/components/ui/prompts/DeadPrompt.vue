...
<template>
  <div class="text-center space-y-6">
    <div class="space-y-4">
      <div class="text-red-400 text-5xl sm:text-6xl">{{ GAME_CONSTANTS.PROMPT_ICONS.DEAD }}</div>
      
      <div class="space-y-3">
        <h3 class="text-xl sm:text-2xl font-bold text-neutral-100">
          {{ GAME_CONSTANTS.PROMPT_MESSAGES.DEAD.title }}
        </h3>
        <div class="w-16 h-1 bg-gradient-to-r from-red-500 to-red-400 mx-auto rounded-full"></div>
        <p class="text-neutral-400 text-sm sm:text-base max-w-md mx-auto leading-relaxed">
          {{ GAME_CONSTANTS.PROMPT_MESSAGES.DEAD.subtitle }}
        </p>
      </div>
    </div>
    ...
    >
      {{ GAME_CONSTANTS.PROMPT_MESSAGES.DEAD.buttonText }}
    </button>
  </div>
</template>
